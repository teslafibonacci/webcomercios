<?php
    include_once("../config.php");
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <title>Comercio</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
</head>

<body>

    <nav class="navbar navbar-expand-sm bg-primary navbar-dark">
        <div class="container-fluid">
            <a class="navbar-brand" href="../index.php">Comercio</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#collapsibleNavbar">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="collapsibleNavbar">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" href="altaComercio.php">Alta Comercio</a>
                    <li class="nav-item">
                        <a class="nav-link" href="bajaComercio.php">Baja Comercio</a>
                    </li>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="consultaComercio.php">Consulta Comercio</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="modificaComercio.php">Modifica Comercio</a>
                    </li>
                </ul>
            </div>
        </div>
    </nav>

    <div class="container mt-3">
        <h2>Comercio</h2>
        <form method="post" action="guardaComercio.php">
            <div class="mb-3 mt-3">
                <label for="comercio"> <strong>Comercio:</strong></label>
                <input type="text" class="form-control" id="comercio" placeholder="Ingresa el nombre del comercio"
                    name="comercio">
            </div>
            <button type="submit" class="btn btn-primary">Guardar Comercio</button>
    </div>
</body>

</html>